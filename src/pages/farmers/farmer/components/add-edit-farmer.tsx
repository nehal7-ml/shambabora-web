...
const AddEditFarmer = ({
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['farmerUser'] })
  ...
  function onSubmit(data: FormSchema) {
    let farmer = {
      ...
      amcosMemberId: data.amcosMemberId,
      amcos: data.amcos,
      ttbNumber: data.ttbNumber,
      tinNumber: data.tinNumber,
      voterId: data.voterId,
      driversLicense: data.driversLicense,
      user: mode === 'edit' ? initialData?.user.id : undefined,
      email: mode === 'add' ? data.email : undefined,
      role: mode === 'add' ? "farmer" : undefined
    }


    const finalData = camelToSnakeCase(farmer);
  ...
  return (
    <Dialog open={true} onOpenChange={handleCancel}>
      <DialogContent className='w-full max-w-7xl py-10 lg:max-h-lvh max-h-svh overflow-auto'>
        <DialogHeader>
          <DialogTitle>
